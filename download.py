...
def download(Qt_Infor, invedio, m3u8, tsUrl, address, id, pinzhi, title):
  ...
  if host == None:
    ...
    Qt_Infor.setText(u'下载视频：' + title + '\n' +
                     u'输出文件：' + fl + '.mp4')
    win32api.ShellExecute(0, 'open', 'ffmpeg.exe', unicode(arg), '', True)
  ...
  # 旧的url使用传统的get方法下载
    ...
    while i < length:
      ...
      if stopDownload == False:
        ...
        Qt_Infor.setText(u'写入文件：' + fl + '.ts' + '\n' +
                         str(i + 1) + '/' + str(length) + ' ' + host + tsUrl[i] + '.ts ' + code)
        i += 1
        
        if i == length:
          Qt_Infor.setText(u'下载完成：' + fl + '.ts' + '\n' +
                           str(i + 1) + '/' + str(length) + ' ' + host + tsUrl[i] + '.ts')
      else:
        Qt_Infor.setText(u'停止下载：' + fl + '.ts' + '\n' +
                         str(i + 1) + '/' + str(length) + ' ' + host + tsUrl[i] + '.ts')
        break
